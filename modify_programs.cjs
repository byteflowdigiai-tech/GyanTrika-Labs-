...
const filePath = 'src/data/programsData.ts';
let content = fs.readFileSync(filePath, 'utf-8').replace(/\r\n/g, '\n');

// Update Program interface
const interfaceReplacement = `export interface Program {
    id: string;
    title: string;
...
// Tag remaining with pg
content = content.replace(/(id: "prog-00\d+",\n\s+title: ".*?",\n\s+category: ".*?",)/g, '$1\n        educationLevel: "pg",');
...
console.log('Done modifying programsData.ts');
